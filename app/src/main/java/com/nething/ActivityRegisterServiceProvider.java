...
public class ActivityRegisterServiceProvider extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        serviceprovider_Login.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                startActivity(new Intent(ActivityRegisterServiceProvider.this,ActivityLogInServiceProvider.class));
            }
        });
        //------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------//
                ...
                if(validate())
                {
                    ...
                    mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(task -> {
                        if (task.isSuccessful()){
                            Intent intent = new Intent(ActivityRegisterServiceProvider.this, ActivityServiceProviderDetails.class);
                            intent.putExtra("name", name);
                            intent.putExtra("phone", phone);
                            intent.putExtra("email", email);
                            startActivity(intent);
                            Toast.makeText(ActivityRegisterServiceProvider.this, "Registered Successfully!", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(ActivityRegisterServiceProvider.this, "Sorry! Failed to register", Toast.LENGTH_SHORT).show();
                        }
                    }).addOnFailureListener(e -> {
                        Toast.makeText(ActivityRegisterServiceProvider.this, e.toString(), Toast.LENGTH_SHORT).show();
                    });
